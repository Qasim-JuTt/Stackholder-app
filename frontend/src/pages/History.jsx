    ...
    try {
      ...
      if (Array.isArray(data?.pageVisits)) {
        data.pageVisits.forEach((visit) => {
          const [date, time] = visit.timestamp?.split('T') || ['Unknown', 'Unknown'];
          combined.push({
            id: visit._id,
      ...
      if (Array.isArray(data?.changeLogs)) {
        data.changeLogs.forEach((log) => {
          const [date, time] = log.updatedAt?.split('T') || ['Unknown', 'Unknown'];
          combined.push({
            id: log._id,
            user: log.updatedBy || 'Unknown',
            action: log.operation?.charAt(0).toUpperCase() + log.operation?.slice(1),
            page: log.model,
            description: `${log.updatedBy || 'Unknown'} performed ${log.operation} on ${log.model}`,
            date,
            time: time?.split('.')[0] || 'Unknown',
          });
        });
      ...
      combined.sort((a, b) => new Date(`${b.date}T${b.time}`) - new Date(`${a.date}T${a.time}`));
      setLogs(combined);
    } catch (error) {
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredLogs.map((log, index) => (
                    <tr key={log.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="py-4 pr-4 whitespace-nowrap">{log.date}</td>
                      <td className="py-4 pr-4 whitespace-nowrap">{log.time}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
